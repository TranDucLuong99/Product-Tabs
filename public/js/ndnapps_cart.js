...
!(function(){
    ...
    var ndnappsBlogtestJavaScript = function(jQuery){
        ...
        var blogtest_data_response = JSON.parse(B64.decode(ndn_cart_data));

        //add button to theme
        var cart_data_response = JSON.parse(B64.decode(ndn_cart_status));
        if (cart_data_response  == 1) {
            $( "div.product-form__item" ).prepend("<div class='ndnapps-cart-wrapper' id='ndnapps-cart-wrapper'></div>" );

            $( "button.product-form__cart-submit" ).remove();
        }else {
            $( ".ndnapps-cart-wrapper" ).remove();
        }
                ...
                var frag = (function(){
                    ...
                    if(itemId){
                        //wrap.innerHTML = '<div class="ndnapps-blogtest" id="ndnapps-blogtest-'+itemId+'"></div>';
                            ...
                            wrap.innerHTML = ('<div class="ndnapps-cart-wrapper" id="ndnapps-cart-wrapper"></div>');
                    }
        ...
        if(jQuery('#ndnapps-cart-wrapper').length){
            jQuery("#ndnapps-cart-wrapper").append(blogtest_data_response);
        }
